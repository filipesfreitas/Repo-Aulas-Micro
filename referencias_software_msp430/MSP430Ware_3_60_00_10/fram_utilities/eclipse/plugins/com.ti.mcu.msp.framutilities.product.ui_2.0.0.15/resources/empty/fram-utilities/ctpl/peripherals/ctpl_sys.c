...
void ctpl_SYS_save(uint16_t baseAddress, uint16_t *storage, uint16_t mode)
{
    ...
    storage[6] = HWREG16(baseAddress + OFS_SYSCFG2);
    storage[5] = HWREG16(baseAddress + OFS_SYSCFG1);
    storage[4] = HWREG16(baseAddress + OFS_SYSCFG0);
#endif
...
#if defined(__MSP430FR2XX_4XX_FAMILY__)
#if defined(FRWPPW)
    HWREG16(baseAddress + OFS_SYSCFG0) = (storage[4] & 0x00ff) | FRWPPW;
#else
    HWREG16(baseAddress + OFS_SYSCFG0) = storage[4];
#endif
    HWREG16(baseAddress + OFS_SYSCFG1) = storage[5];
    HWREG16(baseAddress + OFS_SYSCFG2) = storage[6];
#endif
