...
// Returns a pointer to the report structure that's been built and is ready to
// send.
BYTE* KB_getReportPtr(void)
{
	return (BYTE*) keyUnion.keyArray;
}
...
// Adds the specified key (printing, non-printing, or modifier) to the
// persistent key report.
size_t KB_addKeypressToReport(uint8_t k)
{
	uint8_t i;
...
// Takes the specified key out of the persistent key report.  Once sent, this
// tells the OS the key is no longer pressed and that it shouldn't be repeated
// anymore.
size_t KB_addKeyReleaseToReport(uint8_t k)
{
	uint8_t i;
	if (k >= 136) {			// it's a non-printing key (not a modifier)
		k = k - 136;
